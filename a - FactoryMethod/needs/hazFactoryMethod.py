...
class BadTemperament(Mind):# a PRODUCT - keep it!
    def __str__(self):
        return "Puppy dog tails!"

class Baby(metaclass=ABCMeta): 
    @abstractmethod
    def makeAttitude(self): pass # FACTORY METHOD
    @abstractmethod
    def makeTemperament(self): pass # FACTORY METHOD
...
class ChallengingBaby(Baby): # a CREATOR class(in the sense that creating a baby creates a mind)
    def makeAttitude(self):
        return BadAttitude()
    def makeTemperament(self):
